...
Before making any changes, it's a good idea to take a snapshot of your system if you're using ZFS. This way, you can easily revert back if something goes wrong during the process. To create a new snapshot, use the following command:
...
#+begin_src sh
./configure --prefix="$HOME/.local/emacs/" --without-compress-install --with-native-compilation --with-json --with-mailutils --with-tree-sitter CC=gcc-12 ac_cv_header_sys_sysinfo_h=no
#+end_src
