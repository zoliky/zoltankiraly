...
*bold* /italic/ _underscore_ ~code~ =verbatim= +strikethrough+
...
Taking snapshots uses virtually no space and is completed in a fraction of a second. If you need to go back, simply select the snapshot in the FreeBSD boot menu, which is the first menu that shows up after you reboot your computer.
...
You can remove the snapshot if you wish, although it's not necessary. As mentioned earlier, snapshots take up virtually no space, and you can have thousands of them without any problem.

#+begin_src sh
# bectl destroy 2024-aug-04-before-installing-emacs
#+end_src

I will extend this blog post to include instructions on how to create desktop icons for easier access.

#+begin_quote
This is a quote. Let's see how it looks.

Second line in the quote
#+end_quote
